...
'Line 14 to where you'd like the folder to be stored, complete with the "\" at the end. For the sake of convenience, it's suggested that you store it in the same path that the zip folder was stored, but it makes no difference since both the zip file and extracted folder will be deleted by the end of this program. 
...
'Lines 29 and 30 to the path of the previously made temporary folder (as referenced in line 4) plus the path of the zip file. For example, if this program just extracted file.zip to ./folder, then set the target path to ./folder/file
...
fso.CopyFile "[REPLACE THIS PATH, SEE LINE 5]*.*", "[REPLACE THIS PATH, SEE LINE 6]",true
